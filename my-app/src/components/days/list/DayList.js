...
export default function DayList({ dayList, setDayList, tripId, trip }) {
    ...
    if (dayList.length !== totalDays) {
        button = <button onClick={handleDayAdd} type="button" className={`btn btn-primary btn-m ${classes.button}`}>Add a day</button>;
    } else {
        button = <button hidden onClick={handleDayAdd} type="button" className={`btn btn-primary btn-m ${classes.button}`}>Add a day</button>;
    }
